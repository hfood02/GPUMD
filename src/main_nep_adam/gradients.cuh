...
struct Gradients {
  void resize(int N, int max_NN_radial, int num_variables, int number_of_variables_ann, int dim, int basis_size_radial) {
    ...
    q_c.resize(N * max_NN_radial * (basis_size_radial + 1));
  ...
  GPU_Vector<float> q_c;        // partial descriptors / partial C
};